...
class Server:
    ...
    def init(self):
        ...
        # TODO: put 160 in production, made for debug purposes
    ...
    def loop(self):
        ...
        iterator = RawDataIterator(self.h5file, shuffle=self.shuffle,
        ...
        while True:
            ...
            print("%s: generation %s, %d images " %
                  (self.name, generation, keys))
            ...
            for (image, mask, labels, keypoints) in iterator.gen():
                ...
                num += 1
                print("%s [%d/%d] aug %0.2f ms (%0.2f im/s), send %0.2f s" %
                      (self.name, num, keys, augment_time*1000,
                       1./augment_time,  time() - start - augment_time))
                start = time()

    def produce_headers(self, img, mask, labels, keypoints):

        header_data = {"descr": img.dtype.str, "shape": img.shape,
                       "fortran_order": False}
        header_mask = {"descr": mask.dtype.str, "shape": mask.shape,
                       "fortran_order": False}
        header_label = {"descr": labels.dtype.str, "shape": labels.shape,
                        "fortran_order": False}
        header_keypoints = {"descr": keypoints.dtype.str,
...
np.set_printoptions(precision=1, linewidth=100*3,
