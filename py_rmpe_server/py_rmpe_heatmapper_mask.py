...
class Heatmapper:
    ...
    def create_heatmaps(self, joints, mask):
        ...
        heat_start = RmpeGlobalConfig.heat_start
        heat_layers = RmpeGlobalConfig.heat_layers

        self.put_joints(heatmaps, joints)
        sl = slice(heat_start, heat_start + heat_layers)
        ...
        # heatmaps[heat_start:heat_layers, :, :] = \
        #     (heatmaps[heat_start:heat_layers, :, :] > 0.5).astype(np.int_)
        heatmaps *= mask
        return heatmaps
    ...
    def put_vector_maps(self, heatmaps, layer_x, layer_y, joint_from, joint_to):
        ...
        for i in range(joint_from.shape[0]):
            ...
            if dnorm == 0:  # We get nan here sometimes, it's kills NN.
                # TODO: Handle it better. probably we should add zero paf,
                # centered paf, or skip this completely.
    ...
    def put_limbs(self, heatmaps, joints):
        ...
        for (i, (fr, to)) in enumerate(RmpeGlobalConfig.limbs_conn):
            ...
            layer_x, layer_y = (RmpeGlobalConfig.paf_start + i*2,
                                RmpeGlobalConfig.paf_start + i*2 + 1)
            self.put_vector_maps(heatmaps, layer_x, layer_y,
