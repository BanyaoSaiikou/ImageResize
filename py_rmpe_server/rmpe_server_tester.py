...
mask_pattern = np.zeros((RmpeGlobalConfig.height, RmpeGlobalConfig.width, 3),
                        dtype=np.uint8)
heat_pattern = np.zeros((RmpeGlobalConfig.height, RmpeGlobalConfig.width, 3),
                        dtype=np.uint8)
...
def save_images(num, image, mask, paf):
    ...
    mask_img = mask_img.reshape((RmpeGlobalConfig.height,
    ...
    for i in range(RmpeGlobalConfig.num_parts_with_background):
        ...
        heat_img = cv2.resize(heat_img,
                              (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                              interpolation=cv2.INTER_NEAREST)
        heat_img = heat_img.reshape((RmpeGlobalConfig.height,
                                     RmpeGlobalConfig.width, 1))
    ...
    for i, (fr, to) in enumerate(RmpeGlobalConfig.limbs_conn):
        ...
        for x in range(RmpeGlobalConfig.width//stride):
            for y in range(RmpeGlobalConfig.height//stride):
                ...
                if X != 0 or Y != 0:
                    cv2.arrowedLine(paffed_image, (x*stride, y*stride),
                                    (int(x*stride+X*stride),
                                     int(y*stride+Y*stride)),
...
def time_processed(client, batch_size):
    ...
    for x, y in client.gen():
        num += 1
        elapsed = time() - start
        print(num*batch_size, num*batch_size/elapsed,
              [i.shape for i in x], [i.shape for i in y])
...
def time_raw(client, save):
    ...
    for foo in client.gen_raw():
        ...
        if len(foo) == 3:
            ...
            raise NotImplementedError("Unknown number of tensors in proto %d"
                                      % len(foo))
...
def main(type, batch_size, save):
    ...
    client = DataGeneratorClient(port=5556, host="localhost",
                                 hwm=1, batch_size=batch_size)
...
temp = "Usage: ./rmpe_dataset_server_tester <processed|raw> [batch_size] [save]"
assert len(sys.argv) >= 2, temp
batch_size = 1
save = False
if "save" in sys.argv:
    save = True
