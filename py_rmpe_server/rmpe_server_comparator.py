...
clients = {}
# Save new server to output, c++ server to original and compare images
save_to = "comparator"
...
def cmp_pics(num, lhsd, rhsd, lhsn, rhsn):
    ...
    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "image." + lhsn),
                lhsd.transpose([1, 2, 0]))
    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "image." + rhsn),
                rhsd.transpose([1, 2, 0]))
    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "imagediff"),
                diff)
...
def cmp_masks(num, lhsd, rhsd, lhsn, rhsn):
    ...
    lhsd = cv2.resize(lhsd, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
    ...
    rhsd = cv2.resize(rhsd, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                      interpolation=cv2.INTER_NEAREST)
    rhsd = rhsd.reshape((RmpeGlobalConfig.height, RmpeGlobalConfig.width, 1))

    diff = np.abs(diff).reshape((diff.shape[0], diff.shape[1], 1))
    diff = diff.astype(np.uint8)
    diff = cv2.resize(diff, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                      interpolation=cv2.INTER_NEAREST)
    diff = diff.reshape((RmpeGlobalConfig.height, RmpeGlobalConfig.width, 1))

    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "mask."+lhsn),
                lhsd)
    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "mask."+rhsn),
                rhsd)
    cv2.imwrite(save_to+("/%5d" % num)+"/%07d%s.png" % (num, "maskdiff"),
                diff)
...
def cmp_layers(num, lhsd_all, rhsd_all, lhsn, rhsn):
    ...
    for layer in range(RmpeGlobalConfig.num_layers):
        ...
        lhsd = cv2.resize(lhsd,
                          (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                          interpolation=cv2.INTER_NEAREST)
        lhsd = lhsd.reshape((RmpeGlobalConfig.height,
                             RmpeGlobalConfig.width, 1))

        rhsd = rhsd.reshape((rhsd.shape[0], rhsd.shape[1], 1))
        rhsd = (127+rhsd*128).astype(np.uint8)
        rhsd = cv2.resize(rhsd,
                          (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                          interpolation=cv2.INTER_NEAREST)
        rhsd = rhsd.reshape((RmpeGlobalConfig.height,
                             RmpeGlobalConfig.width, 1))

        diff = np.abs(diff).reshape((diff.shape[0], diff.shape[1], 1))
        diff = diff.astype(np.uint8)
        diff = cv2.resize(diff,
                          (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                          interpolation=cv2.INTER_NEAREST)
        diff = diff.reshape((RmpeGlobalConfig.height,
                             RmpeGlobalConfig.width, 1))

        cv2.imwrite(save_to + ("/%5d" % num)
                    + "/%07d%s.png" % (num, "layer" + str(layer) + "." + lhsn),
                    lhsd)
        cv2.imwrite(save_to + ("/%5d" % num)
                    + "/%07d%s.png" % (num, "layer" + str(layer) + "." + rhsn),
                    rhsd)
        cv2.imwrite(save_to + ("/%5d" % num)
                    + "/%07d%s.png" % (num, "layer" + str(layer) + "diff"),
                    diff)

        result += [L1, L2, AC]

    print("Layers: ", num, lhsd.shape, rhsd.shape,
          L1T/RmpeGlobalConfig.num_layers, L2T/RmpeGlobalConfig.num_layers,
          ACT/RmpeGlobalConfig.num_layers)
...
def main(servers, batch_size):
    ...
    for (name, host, port) in servers:
        clients[name] = DataGeneratorClient(port=port, host=host, hwm=1,
                                            batch_size=batch_size).gen_raw()
    ...
    for layer in range(RmpeGlobalConfig.num_layers):
        columns += ["Layer"+str(layer)+"L1", "Layer"+str(layer)+"L2",
                    "Layer"+str(layer)+"AC"]
