...
class DataIteratorBase:
    def __init__(self, batch_size=10):
        ...
        self.split_point = RmpeGlobalConfig.heat_start  # 38
        self.vec_num = 0  # 38
        self.heat_num = RmpeGlobalConfig.num_parts_with_background  # 19

        # This is not passed to NN, will be accessed by accuracy calculation
    ...
    def gen(self):
        batches_x, batches_x2, batches_y2 = [None]*self.batch_size, \
        ...
        for foo in self.gen_raw():
            ...
            if len(foo) == 5:
                data_img, mask_img, label, label_img, kpts = foo
            else:
            ...
            # vec_weights = np.repeat(mask_img[:, :, np.newaxis],
            #                         self.vec_num, axis=2)
            heat_weights = np.repeat(mask_img[:, :, np.newaxis],
                                     self.heat_num, axis=2)
            # print(heat_weights.shape)
            # batches_x1[sample_idx] = vec_weights[np.newaxis, ...]
            batches_x2[sample_idx] = heat_weights[np.newaxis, ...]
            # print(batches_x2[sample_idx].shape)
            ...
            # heat_label_img = label_img[self.split_point:, :, :]
            # heat_label_img = np.transpose(heat_label_img, (1, 2, 0))
            # plt.figure(1)
            # plt.subplot(211)
            # plt.imshow(label_img)
            # plt.subplot(212)
            # plt.imshow(heat_label[:, :, 0])
            # plt.show()
            ...
            if sample_idx == self.batch_size:
                ...
                #     [batch_y2, batch_y2, batch_y2,
                #      batch_y2, batch_y2, batch_y2]
                yield [batch_x], \
                    [batch_y2, batch_y2, batch_y2, batch_y2, batch_y2, batch_y2]
...
class DataGeneratorClient(DataIteratorBase):
    ...
    def _recv_arrays(self):
        ...
        for header in headers:
            ...
            if isinstance(header['shape'], str):
                array.shape = make_tuple(header['shape'])
            else:
                array.shape = header['shape']
            # This need for comparability with C++ code,
            # for some reasons it is string here, not tuple
...
class DataIterator(DataIteratorBase):
    ...
    def __init__(self, file, shuffle=True, augment=True,
        ...
        self.raw_data_iterator = RawDataIterator(file, shuffle=shuffle,
                                                 augment=augment)
